...
const { fetchSecret, measurandNormalizer } = require('../utils');
...
async function processor(source_name, source) {
    ...
    const sensorReadings = await fetchSensorData(source, apiKey);
    const stations = [];
    ...
    for (const reading of sensorReadings) {
        ...
        for (const lookupEntry of Object.entries(lookup)) {
            ...
            const measure = reading[inputParam];
            const normalizedMeasurand = measurandNormalizer(measurand_unit);

            if ([undefined, null].includes(measure)) continue;

            const sensor = new Sensor({
                sensor_id: `${sensorNode.sensor_node_source_name}-${reading.sensor_index}-${inputParam}`,
                measurand_parameter,
