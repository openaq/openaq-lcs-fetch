...
async function processor(source_name, source) {
    ...
    const stas = [];
    const locs = await locations(source, apiKey);
    ...
    locs.forEach((loc) => {
        for (const m of Object.keys(lookup)) {
            if (loc[m] === undefined || loc[m] === null) continue;

            measures.push(new FixedMeasure({
                sensor_id: `PurpleAir-${loc.sensor_index}-${m}`,
                measure: loc[m],
...
async function locations(source, apiKey) {
    const fields = [
        'sensor_index',
        'location_type',
        'name',
    ...
    const url = new URL(source.meta.url + '/v1/sensors');
