...
async function process(source_name, source) {
    try {
        ...
        console.error('ok - all stations pushed');

        const measures = [`"id","parameter","unit"`];
        locs = locs.filter((loc) => {
            return loc.countsByMeasurement.length;
        }).forEach((loc) => {
            for (const m of loc.countsByMeasurement) {
                measures.push(`"${loc.id}","${m.parameter}","ug/m3"`);
            }
        });

        await Providers.put_measure(source_name, measures);
