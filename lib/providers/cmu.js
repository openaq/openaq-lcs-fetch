...
const { VERBOSE, fetchSecret, measurandNormalizer } = require('../utils');
...
async function processor(source_name, source) {
    ...
    for (const monthDir of monthDirs) {
        if (VERBOSE) console.debug(
            `Searching for relevant files within '${monthDir.name}'...`
        );
        ...
        console.info(
            `Found ${files.length} relevant files to process in '${monthDir.name}'...`
        );
        ...
        for (const file of files) {
            ...
            filesBeingProcessed.push(
                limit(
                    () => processFile({ file, timestamp, stations, source_name, drive })
                )
            );
        }
...
async function processFile({ file, timestamp, stations, source_name, drive }) {
