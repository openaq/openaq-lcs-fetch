...
const secretsManager = new AWS.SecretsManager({
    region: process.env.cmuOauthSecretRegion || process.env.AWS_DEFAULT_REGION || 'us-east-1'
});
...
async function processor(source_name, source) {
    ...
    */
    if (!process.env.cmuOauthSecretArn) throw new Error('cmuOauthSecretArn Env Var Required');

    if (VERBOSE) console.debug('Fetching CMU Google OAuth credentials...');
    const { SecretString } = await secretsManager.getSecretValue({
        SecretId: process.env.cmuOauthSecretArn
    }).promise();
    ...
    await Promise.all(
        monthDirs
            .map(async (monthDir) => {
                ...
                console.info(`Found ${files.length} files to process in '${monthDir.name}'...`);
