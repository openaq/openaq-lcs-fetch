...
async function processor(source_name, source) {
    ...
    const parseMonth = async (monthDir) => {
        ...
        for (const file of files) {
            ...
            for (const row of readings) {
                ...
                for (const lookupEntry of Object.entries(lookup)) {
                    ...
                    const [inputParam, [measurand_parameter, _]] = lookupEntry;
    ...
    await Promise.all(monthDirs.map(parseMonth));
    ...
    await meta.setPreviousIngestionTime(now.toISOString());
    console.log('ok - recorded runtime marker');
}
